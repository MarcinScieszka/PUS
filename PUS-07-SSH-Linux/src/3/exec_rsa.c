...
int authenticate_user(LIBSSH2_SESSION *session, struct connection_data *cd)
{
    ...
    const char *keyfile1 = "./private.key";
    const char *keyfile2 = "./public.key";
    ...
    if (auth_list == NULL)
    {
        /* Jezeli uwierzytelnianie metoda "none" zakonczylo sie powodzeniem: */
    ...
    for (;;) 
    {
        /* Pobranie hasla podanego przy tworzeniu kluczy */
        ...
        /* Uwierzytelnianie za pomoca klucza publiczbego. */
        if (libssh2_userauth_publickey_fromfile(session, cd->username,
                                                keyfile2,
                                                keyfile1,
                                                password) == 0)
        {
